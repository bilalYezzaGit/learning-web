...
export default function SeriePlayerPage({ params }: PageProps) {
  ...
  const [resolvedParams, setResolvedParams] = React.useState<{ parcours: string; id: string } | null>(null)
      ...
      <div className="flex h-[calc(100vh-3.5rem)] flex-col">
        ...
        <header className="flex items-center gap-4 border-b px-4 py-3">
          <Button variant="ghost" size="icon" asChild>
            <Link href={`/${parcours}/reviser/serie/${resolvedParams.id}`}>
              <X className="h-5 w-5" />
        ...
        <div className="flex-1 overflow-auto">
          <div className="mx-auto max-w-3xl px-4 lg:px-6 py-6">
            ...
            <QCMPlayer
              ...
              onExit={() => router.push(`/${parcours}/reviser/serie/${resolvedParams.id}`)}
              showExit={false}
            />
          </div>
        </div>
