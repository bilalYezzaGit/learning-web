...
export default function SerieActivityPage({ params }: PageProps) {
  ...
  const handlePrevious = () => {
    if (prevActivity) {
      router.push(`/reviser/serie/${resolvedParams.id}/${prevActivity.id}`)
    }
  }

  const handleNext = () => {
    if (nextActivity) {
      router.push(`/reviser/serie/${resolvedParams.id}/${nextActivity.id}`)
    }
  }
        ...
        <div className="flex-1 overflow-auto">
          <div className="mx-auto max-w-3xl px-4 py-6 lg:px-6">
            ...
            <QCMPlayer
              ...
              onExit={() =>
                router.push(`/reviser/serie/${resolvedParams.id}`)
              }
              showExit={false}
            />
          </div>
