...
- **Empty / "next"** : Consulter les fiches `_raw/{prog}/sources/*.yaml` et `_raw/{prog}/fondations/` pour identifier le prochain module a transcrire, puis router vers transcribe-module
...
_raw/
  {programme}/               # Ex: 3eme-math/
    pdfs/                    # PDF sources du programme
      Manuel_scolaire_*.pdf
      Corrige_*.pdf
      Parascolaire_*.pdf
      xy_plus_*.pdf
    sources/                 # Fiches d'indexation (1 YAML par PDF)
      ...
      xyplus-3eme-t1.yaml
      ...
    fondations/              # Transcriptions de reference (WF0)
      {module}/
        manuel.typ           # Manuel CNP + Corrige CMS (merge)
        parascolaire.typ     # Kounouz Editions (autonome)
        xyplus.typ           # CAEU Med Ali (autonome)
    enrichissements/         # Transcriptions d'enrichissement (WF1+)
      {module}/
        *.typ                # Series, BAC, devoirs transcrits
  pages/                     # PNG temporaires (gitignored, shared)
```
